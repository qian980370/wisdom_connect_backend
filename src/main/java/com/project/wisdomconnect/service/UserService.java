...
public class UserService extends ServiceImpl<UserMapper, User> implements UserServiceInterface {
    ...
    public Result<?> register(@RequestBody User user){
        ...
        try{
            ...
            if (res != null) { // if there is a duplicated username in database, we refuse register request
            ...
            if (res != null) { // if there is a duplicated email in database, we refuse register request
            ...
            if (res != null) { // if there is a duplicated abn in database, we refuse register request
        ...
        if (!Objects.equals(user.getRole(), "facility")){ //normal user do not need a abn
            if (user.getAbn() != null){
                return Result.error(Constants.CODE_407, Constants.CODE_407_MESSAGE);
            }
        }else { // a facility user need to be checked ABN code by manager
            // fastcode is a verification code which generated by manager after check facility's ABN
            ...
            // Check whether the current verification code belongs to a registered user
            ...
            // pass the validation and update fastcode state
    ...
    public boolean saveUser(User user) {
        // if there is no id in user entity, it means we need to insert new user
        if(user.getId()==null){
            return save(user);
        }else{
            // if there is an id in user entity, and we could find duplicate user in database,
            // we need to update user information rather than insert new user
